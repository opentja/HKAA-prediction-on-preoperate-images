...
def plotImageAndAngle(hipDict, kneeDict, ankleDict, testImagePath, dataSource):
    if not os.path.exists(testImagePath + dataSource + '/legImages/'):
        os.makedirs(testImagePath + dataSource + '/legImages/')
    ...
    for imageName, coordinates in hipDict.items():
        ...
        hipImagePath = testImagePath + 'hip/' + dataSource + '/imagesWithEpochs48Lr0.001BatchSize16Resolution256/' + imageName
        kneeImagePath = testImagePath + 'knee/' + dataSource + '/imagesWithEpochs48Lr0.001BatchSize8Resolution256/' + imageName.replace(
            'hip', 'knee')
        ankleImagePath = testImagePath + 'ankle/' + dataSource + '/imagesWithEpochs48Lr0.001BatchSize8Resolution256/' + imageName.replace(
            'hip', 'ankle')
        ...
        wholeImagePath = testImagePath + dataSource + '/legImages/' + imageName.replace('hip', '')
    ...
    diffStd = statistics.stdev(leftDiffList+rightDiffList)
    diffMean = statistics.mean(leftDiffList+rightDiffList)
    print('mean of difference is: ', diffMean)
    print('std of difference is: ', diffStd)
    # print(len(leftAngleLabelList), len(leftAnglePredList), len(rightAngleLabelList), len(rightAnglePredList))
    np.save(testImagePath + dataSource + '/leftAngleLabelList.npy', leftAngleLabelList)
    np.save(testImagePath + dataSource + '/rightAngleLabelList.npy', rightAngleLabelList)
    np.save(testImagePath + dataSource + '/leftAnglePredList.npy', leftAnglePredList)
    np.save(testImagePath + dataSource + '/rightAnglePredList.npy', rightAnglePredList)


def plotStatisticalFigure(npyFilePath, dataSource):
    leftAngleLabelList = np.load(npyFilePath + dataSource + '/leftAngleLabelList.npy')
    leftAnglePredList = np.load(npyFilePath + dataSource + '/leftAnglePredList.npy')
    rightAngleLabelList = np.load(npyFilePath + dataSource + '/rightAngleLabelList.npy')
    rightAnglePredList = np.load(npyFilePath + dataSource + '/rightAnglePredList.npy')
    # print(len(leftAngleLabelList), len(leftAnglePredList), len(rightAngleLabelList), len(rightAnglePredList))
    ...
    ax[1].legend()
    fig.savefig(npyFilePath + dataSource + '/angleDistribution')
    ...
    ax[1].legend()
    fig.savefig(npyFilePath + dataSource + '/angleDistributionCo')
    plt.subplots_adjust(wspace=1)
    plt.close()
...
plotImageAndAngle(hipDict, kneeDict, ankleDict, testImagePath, dataSource)
